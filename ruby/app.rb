...
module Isucon4
  class App < Sinatra::Base
    use Rack::Session::Redis, secret: ENV['ISU4_SESSION_SECRET'] || 'shirokane'
    use Rack::Flash
    ...
    helpers do
      ...
      def db
        Thread.current[:isu4_db] ||= Mysql2::Client.new(
          socket: ENV['ISU4_DB_SOCKET'] || '/var/lib/mysql/mysql.sock',
      ...
      def last_login
        ...
        db.xquery('SELECT * FROM login_log WHERE succeeded = 1 AND user_id = ? ORDER BY id DESC LIMIT 1', current_user['id']).each.last
      end
